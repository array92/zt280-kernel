	...
	unsigned int cmd, unsigned long arg)
{
	...
	case ECOMPASS_IOC_SET_APARMS:
		if (copy_from_user(parms, pa, sizeof(parms)))
			return -EFAULT;
		/* acceleration x-axis */
		input_set_abs_params(ecs_data_device, /*ABS_X*/ABS_HAT1X, 
			parms[0], parms[1], parms[2], parms[3]);
		/* acceleration y-axis */
		input_set_abs_params(ecs_data_device, /*ABS_Y*/ABS_HAT1Y, 
			parms[0], parms[1], parms[2], parms[3]);
		/* acceleration z-axis */
		input_set_abs_params(ecs_data_device, /*ABS_Z*/ABS_THROTTLE, 
			parms[0], parms[1], parms[2], parms[3]);
		break;
	...
	case ECOMPASS_IOC_SET_YPR:
		...
		if (atomic_read(&a_flag)) {
			input_report_abs(ecs_data_device, /*ABS_X*/ABS_HAT1X, ypr[0]);
			input_report_abs(ecs_data_device, /*ABS_Y*/ABS_HAT1Y, ypr[1]);
			input_report_abs(ecs_data_device, /*ABS_Z*/ABS_THROTTLE, ypr[2]);
