...
static int mma7660_i2c_tx_data(char *buf, int len)
{
	...
	for (i = 0; i < MMA7660_RETRY_COUNT; i++) {
	...
	if (i >= MMA7660_RETRY_COUNT) {
		pr_err("%s: retry over %d\n", __FUNCTION__, MMA7660_RETRY_COUNT);
		return -EIO;
	}
...
static int mma7660_i2c_rx_data(char *buf, int len)
{
	uint8_t i;
	struct i2c_msg msgs[] = {
		{
...
static int reg_read_n(u8 reg,u8 * data,int n)
{
    ...
    ret = mma7660_i2c_rx_data(data, n);
	if (ret != n){
...
static struct dev_pm_ops mma7660_i2c_pm_ops = {
