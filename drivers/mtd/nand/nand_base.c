			    ...
			    struct mtd_oob_ops *ops)
{
	...
	while(1) {
        ...
        */
        chip->ecc.read_oob(mtd, chip, page, readlen);
		//sndcmd = chip->ecc.read_oob(mtd, chip, page, sndcmd);
		    ...
		    int allowbbt)
{
	...
	/* forbid erase block 0 */
	if (instr->addr < mtd->writesize) {
		DEBUG(MTD_DEBUG_LEVEL0, "%s: forbid erase block 0 address 0x%08x %08x\n", __func__,(__u32)(instr->addr>>32),(__u32)(instr->addr));
		return -EINVAL;
	}
