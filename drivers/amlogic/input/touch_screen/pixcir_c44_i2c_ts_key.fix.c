...
int pixcir_ts_suspend(struct pixcir_i2c_ts_data *tsdata,int suspend)
{
    ...
    if(suspend)
        ...
        value = 0;    //active
    ret = i2c_smbus_write_i2c_block_data(tsdata->client, 0x14, 1, &value);
    if(ret){
...
int pixcir_ts_int_init(struct pixcir_i2c_ts_data *tsdata)
{
    ...
    value = 0xa;  //low level
    ret = i2c_smbus_write_i2c_block_data(tsdata->client, 0x15, 1, &value);
    if(ret){
        printk(KERN_ERR "pixcir_ts_init failed %d\n",ret);
    }
...
static int pixcir_ts_poscheck(struct pixcir_i2c_ts_data *data)
{
    ...
    int touching = 0, oldtouching;
	int pos[4];
	u8 rdbuf[11];
#if defined(CONFIG_TOUCHSCREEN_TS_KEY)    
    int key_status = 0;
    int point_status[2] = {0};
#endif
    ...
    //touch event
	ret = i2c_smbus_read_i2c_block_data(tsdata->client, 0x0, 10, &rdbuf[0]);
    if(ret == 10){
    	oldtouching = rdbuf[1];
    	pos[0] = ((rdbuf[3] << 8) | rdbuf[2]);
    	pos[1] = ((rdbuf[5] << 8) | rdbuf[4]);
    	pos[2] = ((rdbuf[7] << 8) | rdbuf[6]);
    	pos[3] = ((rdbuf[9] << 8) | rdbuf[8]);
...
#if defined(DEBUG)            
        printk(KERN_INFO "pixcir %d level %d(%d,%d) (%d,%d)\n",touching,tsdata->chip->get_irq_level(),pos[0],pos[1],pos[2],pos[3]);
#endif
    }else{
		dev_err(&tsdata->client->dev, "pixcir unable to read i2c page 0 ret %d\n",ret);
    }
    ...
    if(touching){
        for(i = 0; i < touching; i++) {
            ...
            if(pdata->convert2)
                pdata->convert2(&pos[(i<<1)],&pos[(i<<1)+1]);

#if defined(CONFIG_TOUCHSCREEN_TS_KEY)
            if(pdata->to_key)
                point_status[i] = pdata->to_key(pdata->key,pdata->key_num,pos[(i<<1)],pos[(i<<1)+1]);
            ...
            input_report_abs(tsdata->input, ABS_MT_POSITION_X, pos[(i<<1)]);
            input_report_abs(tsdata->input, ABS_MT_POSITION_Y, pos[(i<<1)+1]);
    ...
    //key event
#if defined(CONFIG_TOUCHSCREEN_TS_KEY)        
	ret = i2c_smbus_read_i2c_block_data(tsdata->client, 41, 1, &rdbuf[10]);
    if(ret == 1){
        if(rdbuf[10] && rdbuf[10] <= 0xF){
            key_status |= rdbuf[10];
            touching ++;
#if defined(DEBUG)
            printk(KERN_INFO "pixcir %d level %d key 0x%x\n",touching,tsdata->chip->get_irq_level(),rdbuf[10]);
#endif
        }
    }else{
        dev_err(&tsdata->client->dev, "pixcir unable to read i2c page 41 ret %d\n",ret);
    }
    tsdata->key_status = 
...
static int pixcir_ts_poscheck(struct pixcir_i2c_ts_data *data)
{
	...
	ret = i2c_smbus_read_i2c_block_data(tsdata->client, PROTOCOL_ADDR + I2C_STARTTCH_READ, I2C_STARTTCH_POINT2 - I2C_STARTTCH_READ, &rdbuf[I2C_STARTTCH_READ]);
    if(ret == (I2C_STARTTCH_POINT2 - I2C_STARTTCH_READ) &&
    ...
    if(touching){
        ...
        if(touching<POINTS_MAX)
            touching++;
    
        if(touching >= 2){
            //touch event(get point left)
            ret = i2c_smbus_read_i2c_block_data(tsdata->client, PROTOCOL_ADDR + I2C_STARTTCH_POINT2, (touching - 2) * PROTOCOL_POINT_LEN, &rdbuf[I2C_STARTTCH_POINT2]);
            if(ret == (touching - 2) * PROTOCOL_POINT_LEN){
            }else{
...
static void pixcir_i2c_late_resume(struct early_suspend *h) {
    ...
    ts->suspend = 0;
    /*
    pixcir_ts_int_init(ts);
    pixcir_ts_suspend(ts,0);
    msleep(10);
    */
