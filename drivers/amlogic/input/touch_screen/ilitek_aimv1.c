	...
	int max_y)
{
	...
	for(key=0; key<ARRAY_SIZE(touch_key_code); key++){
	...
	void)
{
	...
	//printk("ilitek_i2c_process_and_report 0x%X, buf[0] = %d\n",i2c.protocol_ver,buf[0]);

	//input_report_key(i2c.input_dev, BTN_TOUCH, buf[0]);
	// multipoint process
	if(i2c.protocol_ver & 0x200){
		...
		tp_id = buf[0];
		key_id = buf[1] - 1;
		//printk("ilitek_i2c_process_and_report tp_id %d\n",tp_id);
		...
		default:
			//printk("litek_i2c_process_and_report i2c.max_tp is %d\n",i2c.max_tp);
			for(i=0; i<i2c.max_tp; i++){
				...
				//printk("ilitek_i2c_process_and_report x %d   y %d i %d\n",x,y,i);
...
{	
	...
	do {
		ilitek_i2c_process_and_report();
		i2c.attb_pin = i2c.pdata->get_irq_level();
		
		if (i2c.attb_pin == 0) {
			// wait_event_timeout(i2c.wait, 1,msecs_to_jiffies(50));
			msleep(5);
		}
	} while (!i2c.attb_pin);
	...
	const struct i2c_device_id *id)
{
	...
	i2c.client = client;
	//i2c.pdata  = (struct i2c_data*)i2c.client->dev.platform_data;
    i2c.pdata  = i2c.client->dev.platform_data;
	ret = i2c.pdata->init_irq();
    if(ret<0){
	...
	void)
{
	...
	if (request_threaded_irq(i2c.client->irq, NULL, ilitek_i2c_isr,
			IRQF_TRIGGER_FALLING, "ilitek_i2c_irq", &i2c)) {
		dev_err(&i2c.client->dev, "Unable to request touchscreen IRQ.\n");
