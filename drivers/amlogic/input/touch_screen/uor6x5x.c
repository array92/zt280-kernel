...
#define KBD_POINT_WORK_QUEUE_DELAY 100
...
#define FIRST_ONE_TOUCH_AFTER_KBD_FILTER    1
...
#define FIRST_TWO_TOUCH_AFTER_KBD_FILTER    FIRST_TWO_TOUCH_AFTER1_FILTER

#define FIRST_KBD_TOUCH_AFTER0_FILTER	3
#define FIRST_KBD_TOUCH_AFTER1_FILTER	2
#define FIRST_KBD_TOUCH_AFTER2_FILTER	2
#define FIRST_KBD_TOUCH_AFTER_KBD_FILTER    1
...
#define MAX_KBD_PENDING_POINT   1
...
struct uor6x5x_touch_screen_struct {
    ...
    //kbd touch cache   
    struct ts_sample pending_sample_kbd[MAX_PENDING_POINT];
    int pending_kbd_touch;
    int pending_kbd_touch_ns;
    int pending_kbd_touch_nr;
    int kbd_value[TS_KEY_MAX];
...
static void uor_cache_reset(struct uor6x5x_touch_screen_struct *t)
{
    ...
    t->pending_zero_touch_nr = t->pending_one_touch_nr = t->pending_two_touch_nr = t->pending_kbd_touch= 0;
    t->pending_one_touch_ns = t->pending_two_touch_ns = t->pending_kbd_touch_ns = 0;
    t->pending_one_touch = t->pending_two_touch = t->pending_kbd_touch = 0;
...
int is_ts_key(struct uor6x5x_touch_screen_struct *t,const struct ts_sample *s)
{
#if defined(CONFIG_TOUCHSCREEN_TS_KEY)

    struct touch_platform_data *pdata = t->pdata;
    int i;

    for (i=0; i<pdata->key_num; i++) {
        if((s->x > pdata->key[i].x0 && s->x <pdata->key[i].x1)&&
            (s->y > pdata->key[i].y0 && s->y <pdata->key[i].y1)){
#if defined(DEBUG)                             
            //printk(KERN_INFO "It's %s key(%d)\n", pdata->key[i].name, pdata->key[i].code);
#endif
            return 0;
        }
    }
...
int ts_sample_to_key(struct uor6x5x_touch_screen_struct *t,const struct ts_sample *s)
{
#if defined(CONFIG_TOUCHSCREEN_TS_KEY)
    ...
    for (i=0; i<pdata->key_num; i++) {
        val = 0;
        if(s){ //null mean release all key
            if((s->x > pdata->key[i].x0 && s->x <pdata->key[i].x1)&&
                (s->y > pdata->key[i].y0 && s->y <pdata->key[i].y1)){
#if defined(DEBUG)                                 
        ...
        if(val != t->kbd_value[i]){
            t->kbd_value[i] = val;
            printk(KERN_INFO "%s key(%d) %d\n", pdata->key[i].name, pdata->key[i].code,val);
            input_event(t->dev_kbd, EV_KEY, pdata->key[i].code,val);
        }
    }
#endif
    return 0;
...
static int uor_report_pending_points(struct uor6x5x_touch_screen_struct *t,int flags)
{
    ...
    if(flags & PENDING_KBD_TOUCH){
        ...
        ts_sample_to_key(t,&t->pending_sample_kbd[ns]);
...
int inc_kbd_touch_pending_point(struct uor6x5x_touch_screen_struct *t,struct ts_sample *s)
{
    t->pending_sample_kbd[t->pending_kbd_touch_ns].x = s->x;
    t->pending_sample_kbd[t->pending_kbd_touch_ns].y = s->y;
    t->pending_sample_kbd[t->pending_kbd_touch_ns].p = s->p;
    
    t->pending_kbd_touch++;
    t->pending_kbd_touch_ns++;
    if(t->pending_kbd_touch_ns == MAX_KBD_PENDING_POINT)
        t->pending_kbd_touch_ns=0;
    if(t->pending_kbd_touch_nr < MAX_KBD_PENDING_POINT)
        t->pending_kbd_touch_nr++; 

    return (t->pending_kbd_touch_nr > 0);
}
...
int dec_kbd_touch_pending_point(struct uor6x5x_touch_screen_struct *t)
{
    if(t->pending_kbd_touch_ns==0)
        t->pending_kbd_touch_ns = MAX_KBD_PENDING_POINT-1;
    else
        t->pending_kbd_touch_ns--;
    if(t->pending_kbd_touch_nr) t->pending_kbd_touch_nr--;
    if(t->pending_kbd_touch) t->pending_kbd_touch--;
...
void clr_kbd_touch_pending_point(struct uor6x5x_touch_screen_struct *t)
{
    t->pending_kbd_touch = 0;
    t->pending_kbd_touch_ns=0;
    t->pending_kbd_touch_nr=0;
}
...
static void uor_read_loop(struct work_struct *data)
{
    ...
    do{
...
#if defined(DEBUG)        
            printk(KERN_ERR "0x%x(0x%x) touch: (x,y)=(%d,%d)(%d,%d) (dx,dy)=(%d,%d)(%d,%d) R %d\n", 
                ntouch,t->pendown,
        ...
        if(ntouch != ZERO_TOUCH){	// pen down            
        	...
        	if(ntouch == TWO_TOUCH){
                if((delta.dx > JITTER_THRESHOLD_TWO_TOUCH || delta.dy > JITTER_THRESHOLD_TWO_TOUCH)){
                    ...
                    if(dec_one_touch_pending_point(t)){
                        t->pendown &= ~(PENDING_ONE_TOUCH|PENDING_KBD_TOUCH);
                    }
        	    }else{
                    ...
                    if(distance != t->distance){  //not report the same point in two touch mode
                        ...
                        if(((t->pendown & ZERO_TOUCH) && t->pending_two_touch < FIRST_TWO_TOUCH_AFTER0_FILTER)||
                            ...
                            ((t->pendown & KBD_TOUCH) && t->pending_two_touch < FIRST_TWO_TOUCH_AFTER_KBD_FILTER)){
#if defined(DEBUG) 
                            ...
                            if(inc_two_touch_pending_point(t,&sample)){
                                ...
                                t->pendown &= ~(PENDING_ONE_TOUCH|PENDING_KBD_TOUCH);  //delete one touch pending
                            }
                            ...
                            if(dec_kbd_touch_pending_point(t)){
                                t->pendown &= ~PENDING_KBD_TOUCH;
                            }
                        }else{
#if defined(DEBUG)                     
                            ...
                            if(t->pendown&KBD_TOUCH){   //close single touch after multi touch is work            
                                  ts_sample_to_key(t,NULL);
                            }

                            t->pendown=TWO_TOUCH;
                            clr_one_touch_pending_point(t);
                            clr_kbd_touch_pending_point(t);
                        }
                    }
                }
                ...
                if((t->pendown & ONE_TOUCH)&&
                    ...
                    if((t->pendown & TWO_TOUCH) && t->pending_one_touch < FIRST_ONE_TOUCH_AFTER2_FILTER){
                        ...
                        ((t->pendown & KBD_TOUCH) && (t->pending_one_touch < FIRST_ONE_TOUCH_AFTER_KBD_FILTER))){
#if defined(DEBUG) 
            ...
            }else if(ntouch == KBD_TOUCH){
                if(((t->pendown & ZERO_TOUCH) && t->pending_kbd_touch < FIRST_KBD_TOUCH_AFTER0_FILTER)||
                    ((t->pendown & ONE_TOUCH) && t->pending_kbd_touch < FIRST_KBD_TOUCH_AFTER1_FILTER)||
                    ((t->pendown & TWO_TOUCH) && t->pending_kbd_touch < FIRST_KBD_TOUCH_AFTER2_FILTER)||
                    ((t->pendown & KBD_TOUCH) && t->pending_kbd_touch < FIRST_KBD_TOUCH_AFTER_KBD_FILTER)){

#if defined(DEBUG) 
                    printk(KERN_ERR "pending (kbd) ns %d nr %d\n",t->pending_kbd_touch_ns,t->pending_kbd_touch_nr);
#endif
                    if(inc_kbd_touch_pending_point(t,&sample)){
                        t->pendown |= PENDING_KBD_TOUCH;
                    }
                    if(dec_two_touch_pending_point(t)){
                        t->pendown &= ~PENDING_TWO_TOUCH;
...
#if defined(DEBUG)
                    printk(KERN_ERR "KBD_TOUCH (%d,%d)\n", sample.x, sample.y);
#endif

                    ts_sample_to_key(t,&sample); 
                    
                    t->pendown = KBD_TOUCH;
                    clr_kbd_touch_pending_point(t);
                }
            }else{
                printk(KERN_ERR "uor_read_loop(): n_touch state error ntouch 0x%x !!!\n",ntouch);
            }
        }else {	// pen release
            int freeze_time = 0; 
#if defined(DEBUG)        
...
#endif
            if(t->pendown & (ONE_TOUCH|TWO_TOUCH|KBD_TOUCH|PENDING_ONE_TOUCH|PENDING_TWO_TOUCH|PENDING_KBD_TOUCH)){
            	...
            	if(uor_get_pendown_state(t) && 
                    ...
                    //2 report one release
                    t->pendown |= uor_report_pending_points(t,t->pendown & ~(PENDING_TWO_TOUCH));

                    //3 report one release
                    if(t->pendown & ONE_TOUCH){
                        input_report_abs(t->dev_single, ABS_PRESSURE, 0);
            			input_report_key(t->dev_single, BTN_TOUCH, 0);
            			input_sync(t->dev_single);
                    ...
                    //2 report two press
                    t->pendown |= uor_report_pending_points(t,t->pendown & ~(PENDING_ONE_TOUCH));                    

                	//3 report two release
                    if(t->pendown & TWO_TOUCH){
            	        input_report_abs(t->dev, ABS_MT_TOUCH_MAJOR, 0);
            	        input_mt_sync(t->dev);
            	        input_sync(t->dev);
        ...
        if(t->pendown != ZERO_TOUCH){
            ...
            else if(t->pendown & (KBD_TOUCH|PENDING_KBD_TOUCH))
                queue_time = KBD_POINT_WORK_QUEUE_DELAY;
...
static int uor_register_input_kbd(void)
{
    int ret;
    struct input_dev *	input_device;
#if defined(CONFIG_TOUCHSCREEN_TS_KEY)    
    ...
    input_device->name = "UOR-touch-kbd";
    
    ts.dev_kbd = input_device;
    ts.dev_kbd->evbit[0] = BIT_MASK(EV_KEY)| BIT_MASK(EV_REP);

#if defined(CONFIG_TOUCHSCREEN_TS_KEY)
    for (i=0; i<pdata->key_num; i++) {
        set_bit(pdata->key[i].code, input_device->keybit);
        printk(KERN_INFO "%s key(%d) registed.\n", pdata->key[i].name, pdata->key[i].code);
    }
#endif
    input_device->phys = "UOR-touch-kbd/input0";
    //input_device->dev.parent = &pdata->dev;
    ...
    const struct i2c_device_id *id)
{
	...
	if (uor_register_input_kbd() < 0) {
		dev_err(&client->dev, "register input ts key fail!\n");
		return -ENOMEM;
	}
...
{    
    ts.pendown = TOUCH_UNKNOW;
    queue_delayed_work(queue, &work, HZ/8);
