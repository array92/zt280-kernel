...
{
	if(CONFIG_CONFIG_SDCARD_SLEEP == 10 || CONFIG_CONFIG_SDCARD_SLEEP == 11 ) {	
		return 0;
	}
...
static int pixcir_ts_poscheck(struct pixcir_i2c_ts_data *data)
{
    ...
    if(ret == 10){
        ...
        if(pdata->convert2){
            pdata->convert2(&posx1,&posy1);
            pdata->convert2(&posx2,&posy2);
        }
    	//input_report_key(tsdata->input, BTN_TOUCH, touching);

#if defined(DEBUG)            
        printk(KERN_INFO "pixcir %d (%d,%d) (%d,%d)\n",touching,posx1,posy1,posx2,posx2);
#endif
	...
	ret = i2c_smbus_read_i2c_block_data(tsdata->client, 41, 1, &rdbuf[10]);
    if(ret == 1){
        if(pixcir_ts_key(tsdata,rdbuf[10])){
            touching|=(1<<4);
        }
    }else{
        dev_err(&tsdata->client->dev, "pixcir unable to read i2c page 41 ret %d\n",ret);
			  ...
			  const struct i2c_device_id *id)
{
    ...
    queue_delayed_work(tsdata->queue, &tsdata->work, HZ*10);
...
static void pixcir_i2c_late_resume(struct early_suspend *h) {
    ...
    ts->suspend--;
    enable_irq(ts->irq);
    //queue_delayed_work(ts->queue, &ts->work, HZ/6);
}
#endif
