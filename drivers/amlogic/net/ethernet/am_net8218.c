...
//#define CONFIG_AM_ETHERNET_DEBUG_LEVEL 4
...
//static char DEFMAC[] = "\x00\x01\x23\xcd\xee\x00";
static char DEFMAC[] = {0x00, 0x1B, 0xFC, 0xEB, 0x92, 0xFA};
static char efuse_mac[6];	
extern int get_zt_mac(char *buf);
...
static int phy_reset(struct net_device *ndev)
{
	...
	if (k >= 1000) {
		printk("error to reset mac! 0x%x\n",IO_READ32(np->base_addr + ETH_DMA_0_Bus_Mode));
		//goto error_reset;
	...
	if (k >= 10000) {
		printk("error to reset phy! 0x%x 0x%x\n",
		    mdio_read(ndev, np->phys[0], MII_BMCR),
		    mdio_read(ndev, np->phys[0], MII_PHYSID1));
		goto error_reset;
	}
...
static int setup_net_device(struct net_device *dev)
{
	...
	np->irq_mask= (1 << 16) |       //NIE: Normal Interrupt Summary Enable                                                                     
         	 ...
         	 (1) | 		//tx interrupt
	   	0; 
	get_zt_mac(efuse_mac);
	if( !efuse_mac[0] && !efuse_mac[1] && !efuse_mac[2] && !efuse_mac[3] && !efuse_mac[4] && !efuse_mac[5]) {
		printk("setup_net_device 0 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\r\n"
				,efuse_mac[0],efuse_mac[1],efuse_mac[2],efuse_mac[3],efuse_mac[4],efuse_mac[5]);
		config_mac_addr(dev,DEFMAC);
	} else {
		printk("setup_net_device 1 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\r\n"
				,efuse_mac[0],efuse_mac[1],efuse_mac[2],efuse_mac[3],efuse_mac[4],efuse_mac[5]);		
		config_mac_addr(dev,efuse_mac);
	}
...
extern void  eth_pinmux_init(void);
static int probe_init(struct net_device *ndev)
{
	...
	priv->dev=ndev;
	//ndev->base_addr = (unsigned long)ioremap(0xc9010000,0x2000);
	...
	//printk("probe_init : priv->phys[0] = 0x%x\r\n",priv->phys[0]);
...
static int am_net8218_drv_probe(struct platform_device *pdev){
	am_net_init();
...
static int am_net8218_suspend(struct device *dev)
{
	//struct net_device *ndev = dev_get_drvdata(dev);
	struct net_device *ndev = my_ndev;
	struct am_net_private *np = netdev_priv(my_ndev);
	printk("am_net8218_suspend\r\n");
	mdio_write(ndev, np->phys[0], MII_BMCR, 1<<11);
...
static int am_net8218_resume(struct device *dev)
{
    //struct net_device *ndev = my_ndev;
	//struct net_device *ndev = dev_get_drvdata(dev);
	//struct am_net_private *np = netdev_priv(ndev);
	
	printk(KERN_INFO "am_net8218_resume\n");
	return 0;
}

static const struct dev_pm_ops am_net8218_pm_ops = {
	.suspend	= am_net8218_suspend,
	.resume		= am_net8218_resume,
};

#define am_net8218_PM_OPS (&am_net8218_pm_ops)

#else
#define am_net8218_PM_OPS NULL
#endif


static int  am_net8218_drv_remove(struct platform_device *pdev)
{
	am_net_exit();	
	return 0;
}

static struct platform_driver am_net8218_driver = {
	.probe = am_net8218_drv_probe,
	.remove = __devexit_p(am_net8218_drv_remove),
	.driver = {
		.name	= "am_net8218",
		.owner	= THIS_MODULE,
		.pm	= am_net8218_PM_OPS,
	},
};

/* Entry point for loading the module */
static int __init am_net8218_init_module(void)
{
	return platform_driver_register(&am_net8218_driver);
}

/* entry point for unloading the module */
static void __exit am_net8218_cleanup_module(void)
{
	platform_driver_unregister(&am_net8218_driver);
}

late_initcall(am_net8218_init_module);
module_exit(am_net8218_cleanup_module);
