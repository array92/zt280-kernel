...
*/
static struct nand_ecclayout aml_nand_oob_default = {
	.eccbytes = (__u32)-1,
	.oobfree = {
		{.offset = 0,
		 .length = 8}}
...
{
	struct clk *sys_clk;
	int sys_clk_rate, sys_time, start_cycle, end_cycle, bus_cycle, time_mode, adjust, Tcycle, i;
	...
	for (i=0; i<chip_nr; i++) {
		if (aml_chip->valid_chip[i]) {
			...
			if (aml_chip->ops_mode == AML_MULTI_CHIP_SHARE_RB) {
				...
				status[i] = sndcmd ? (int)chip->read_byte(mtd) : (NAND_STATUS_WP|NAND_STATUS_FAIL);
			}
			if((status[i]&NAND_STATUS_FAIL) || !(status[i]&NAND_STATUS_READY)){
                printk("ce %d cmd %d status 0x%x rb 0x%x byte 0x%x addr 0x%x\n",
                    i,sndcmd,status[i],chip->dev_ready(mtd),chip->read_byte(mtd),aml_chip->page_addr);
            }
			status[0] |= status[i];
		}
	}
...
static void aml_nand_base_command(struct aml_nand_chip *aml_chip, unsigned command, int column, int page_addr, int chipnr)
{
    ...
    DEBUG (MTD_DEBUG_LEVEL2, "aml_nand_base_command: "
              "0x%x col 0x%x page 0x%x ce %d\n", command,column,page_addr,chipnr);
...
static void aml_nand_command(struct mtd_info *mtd, unsigned command, int column, int page_addr)
{
    ...
    DEBUG (MTD_DEBUG_LEVEL2, "aml_nand_command: "
              "command 0x%x page 0x%08x(%d)\n", command ,page_addr,page_addr);
...
static int aml_platform_hwecc_correct(struct aml_nand_chip *aml_chip, unsigned char *buf, unsigned size, unsigned char *oob_buf)
{
	struct nand_chip *chip = &aml_chip->chip;
	struct mtd_info *mtd = &aml_chip->mtd;
	unsigned ecc_step_num,oob_step;
	int error = 0;
    ...
    if(i == chip->ecc.bytes){
        ...
        //printk ("skip no ecc area\n");
        return 0;
    }
    */
    
	 for (ecc_step_num = 0; ecc_step_num < (size / chip->ecc.size); ecc_step_num++) {
        oob_step = ecc_step_num*aml_chip->user_byte_mode;

        /*
	 	for(i = 0; i < aml_chip->user_byte_mode; i++){
            if(oob_buf[oob_step + i] != 0xff)
                break;
	 	}
        if(i == aml_chip->user_byte_mode){
		    //printk("skip ecc at subpage %d\n",ecc_step_num);
            continue;
        }
        */
	 	...
	 	if (NAND_ECC_FAIL(aml_chip->user_info_buf[ecc_step_num]) 
	 		...
	 		printk ("nand communication have uncorrectable ecc error %d oob[%d]0x%x [%d]0x%x\n", 
	 		    ecc_step_num,
	 		    oob_step,
	 		    oob_buf[oob_step],
	 		    oob_step+1,
...
static int aml_nand_read_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf, int page)
{
	...
	if (aml_chip->ops_mode & AML_INTERLEAVING_MODE)
		internal_chipnr = aml_chip->internal_chipnr;
    DEBUG (MTD_DEBUG_LEVEL2, "aml_nand_read_page_hwecc: "
              "page 0x%08x\n", page);
	...
	for (i=0; i<aml_chip->chip_num; i++) {
		...
		if (aml_chip->valid_chip[i]) {
			...
			for (j=0; j<internal_chipnr; j++) {
                ...
                DEBUG (MTD_DEBUG_LEVEL2, "aml nand read page hwecc %d: "
                          "page addr 0x%08x\n", i,page_addr);
				...
				if (aml_chip->plane_num == 2) {
					...
					if (stat < 0) {
						...
						printk("aml nand read data ecc plane0 failed at page %d chip %d internal_chip %d\n", page_addr, i , j);
					}
					else
					...
					if (stat < 0) {
						mtd->ecc_stats.failed++;
						printk("aml nand read data ecc plane1 failed at page %d chip %d internal_chip %d\n", page_addr, i , j);
					}
					else
						mtd->ecc_stats.corrected += stat;
					...
					if (stat < 0) {
						mtd->ecc_stats.failed++;
						printk("aml nand read data ecc failed at page %d chip %d internal_chip %d\n", page_addr, i, j);
					}
					else
						mtd->ecc_stats.corrected += stat;
...
static void aml_nand_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf)
{
    ...
    DEBUG (MTD_DEBUG_LEVEL2, "aml_nand_write_page_hwecc: "
              "page_addr 0x%08x\n", aml_chip->page_addr);
	...
	for (i=0; i<aml_chip->chip_num; i++) {
		...
		if (aml_chip->valid_chip[i]) {
			...
			for (j=0; j<internal_chipnr; j++) {
                ...
                DEBUG (MTD_DEBUG_LEVEL2, "aml nand write page hwecc %d: "
                          "page_addr 0x%08x\n", i,page_addr);
...
static int aml_nand_read_oob(struct mtd_info *mtd, struct nand_chip *chip, int page, int readlen)
{
	...
	unsigned nand_page_size = (1 << chip->page_shift);
	unsigned nand_read_size = (((readlen + aml_chip->user_byte_mode -1) / aml_chip->user_byte_mode) * chip->ecc.size);
    ...
    /*
	if (nand_read_size >= nand_page_size) //at least read one page
	...
	*/
    ...
    DEBUG (MTD_DEBUG_LEVEL2, "aml_nand_read_oob: "
              "page 0x%08x,oob len 0x%x, read data size %d\n", page, readlen, nand_read_size);

	if (chip->cmdfunc == aml_nand_command)
	    chip->cmdfunc(mtd, NAND_CMD_READOOB, 0, page_addr);

	for (i=0; i</*read_chip_num*/aml_chip->chip_num && nand_read_size; i++) {
		if (aml_chip->valid_chip[i]) {
			...
			for (j=0; j<internal_chipnr && nand_read_size; j++) {
				if (j > 0) {
					page_addr = aml_chip->page_addr;
					page_addr |= (1 << aml_chip->internal_chip_shift) * j;
					aml_chip->aml_nand_select_chip(aml_chip, i);
                ...
                DEBUG (MTD_DEBUG_LEVEL2, "aml_nand_read_oob: "
                          "ce %d(internal %d) page_addr 0x%x\n",i, j, page_addr);
				...
				if (aml_chip->plane_num == 2) {
                    ...
                    user_byte_num = dma_once_size/chip->ecc.size*aml_chip->user_byte_mode;
					...
					if (stat < 0) {
						mtd->ecc_stats.failed++;
						printk("read oob ecc plane0 failed at address 0x%08x (page %d) chip: %d\n", page_addr << chip->page_shift, page_addr, i);
					}
					else
						mtd->ecc_stats.corrected += stat;
					...
					if (nand_read_size > 0) {
						...
						if (stat < 0) {
							mtd->ecc_stats.failed++;
							printk("read oob ecc plane1 failed at address 0x%08x (page %d) chip: %d\n", page_addr << chip->page_shift,page_addr,i);
						}
						else
							mtd->ecc_stats.corrected += stat;
                    ...
                    DEBUG (MTD_DEBUG_LEVEL2, "aml_nand_read_oob: "
                              "ce %d(%d plane) len 0x%x\n",i, j, dma_once_size);

					aml_chip->aml_nand_get_user_byte(aml_chip, oob_buffer, user_byte_num);
					stat = aml_chip->aml_nand_hwecc_correct(aml_chip, nand_buffer, dma_once_size, oob_buffer);
					if (stat < 0) {
						mtd->ecc_stats.failed++;
						printk("read oob ecc failed at address 0x%08x (page %d) chip: %d\n", page_addr << chip->page_shift,page_addr,i);
					}
					else
						mtd->ecc_stats.corrected += stat;
...
{ 
    ...
    uint8_t		*oobbuf;
    int page, realpage, chipnr ,user_byte_num;
    int i;
    int32_t ret = 0;

    DEBUG (MTD_DEBUG_LEVEL2,"check bad at %llx(getchip %d)\n", (uint64_t)ofs,getchip);
	...
	if (getchip) {
        ...
        oobbuf = aml_chip->cache_buf + mtd->writesize;

		aml_oob_ops.mode = MTD_OOB_AUTO;
		aml_oob_ops.len = 0;
		aml_oob_ops.ooblen = /*mtd->oobavail*/aml_chip->user_byte_mode;
		aml_oob_ops.ooboffs = chip->ecc.layout->oobfree[0].offset;
		aml_oob_ops.datbuf = NULL;
		aml_oob_ops.oobbuf = oobbuf;

        memset(oobbuf, 0xdd, aml_oob_ops.ooblen);
        ...
        memset(oobbuf, 0xdd, aml_chip->user_byte_mode);
        ret = chip->ecc.read_oob(mtd, chip, page, aml_chip->user_byte_mode);
	}

    if (ret != 0) {
        printk("nand detect bad block at %llx(read oob faided %d)\n", (uint64_t)ofs,ret);
        return ret;
    }
    
    if ((oobbuf[chip->badblockpos] != 0) && (ret == 0))
        return 0;
	...
	if (getchip) {
        ...
        memset(oobbuf, 0xdd, mtd->oobavail);
        ret = chip->ecc.read_oob(mtd, chip, page, mtd->oobavail);
	}
 
    if (oobbuf[chip->badblockpos] == 0) {
        memset(aml_chip->cache_buf, 0, mtd->oobavail); //temp use 'aml_nand_data_buf'
        user_byte_num = ((((1 << chip->page_shift) + chip->ecc.size - 1) / chip->ecc.size) * aml_chip->user_byte_mode);
        
        if (!memcmp(aml_chip->cache_buf, oobbuf, user_byte_num)) {
            printk("nand detect bad block at %llx(marked)\n", (uint64_t)ofs);
            return 1;
        }
    }

    if (mtd->ecc_stats.failed - stats.failed){
        printk("nand detect bad block at %llx(unmarked)\n", (uint64_t)ofs);

        for(i=0;i<mtd->oobavail;i++){        
            if(i && !(i%32)){
                printk("\n");
            }else if(i && !(i%8)){
                printk(" ");
            }
            printk("%02x ",oobbuf[chip->badblockpos+i]);
        }
        printk("\n");        
        //return 1;  //don't report, maybe can be erased
    }
...
{ 
    struct aml_nand_chip *aml_chip = mtd_to_nand_chip(mtd);
	struct nand_chip * chip = mtd->priv;	
	struct erase_info erase_ops;
	struct mtd_oob_ops write_ops;

    int i;
    int ret = 0;

    printk("nand mark bad block at %llx\n", (uint64_t)ofs);
    
    memset(&erase_ops,0,sizeof(erase_ops));
    erase_ops.mtd = mtd;
    erase_ops.addr = ofs;
    erase_ops.len = mtd->erasesize;
    ret = mtd->erase(mtd,&erase_ops);
    if(ret != 0){
        printk("nand mark bad block failed(erase failed %d)\n",ret);
    }
    memset(&write_ops,0,sizeof(write_ops));
	write_ops.mode = MTD_OOB_AUTO;
	write_ops.len = mtd->writesize;
	write_ops.ooblen = mtd->oobavail;
	write_ops.ooboffs = chip->ecc.layout->oobfree[0].offset;
	write_ops.datbuf = aml_chip->cache_buf;
	write_ops.oobbuf = aml_chip->cache_buf + mtd->writesize;
	chip->pagebuf = -1;

	memset(write_ops.datbuf, 0xcc, write_ops.len);
	memset(write_ops.oobbuf, 0x0, write_ops.ooblen);

	ret = mtd->write_oob(mtd, ofs, &write_ops);
    if(ret != 0){
        printk("nand mark bad block failed(write oob failed %d)\n",ret);
    }

    
    ret = aml_nand_block_bad(mtd,ofs,1);
    if(ret > 0){
        printk("nand mark bad block success\n");
        return 0;
    }
    
    /*
    ret = mtd->read_oob(mtd, ofs, &write_ops);
    if(ret != 0){
        printk("nand mark bad block failed(read oob failed %d)\n",ret);
    }
    */

    printk("nand mark bad block failed(check block %d)\n",ret);

    printk("data[0] 0x%02x oob[0] 0x%02x\n",write_ops.datbuf[0],write_ops.oobbuf[write_ops.ooboffs]);
    ...
    for(i=0;i<mtd->oobavail;i++){        
        ...
        printk("%02x ",write_ops.oobbuf[chip->badblockpos+i]);
    }
    printk("\n");
...
static int aml_nand_scan_ident(struct mtd_info *mtd, int maxchips)
{
	...
	for (i = 1; i < maxchips; i++) {
        ...
        id[0] = chip->read_byte(mtd);
        id[1] = chip->read_byte(mtd);

		if (nand_maf_id != /*chip->read_byte(mtd)*/id[0] || aml_type->id[1] != /*chip->read_byte(mtd)*/id[1]){
		//if (nand_maf_id != dev_id[0] || aml_type->id[1] != dev_id[1])
            printk("nand id dismatch 0x%x(0x%x) 0x%x(0x%x)\n",nand_maf_id,id[0],aml_type->id[1],id[1]);
...
ecc_unit_change:
	...
	printk("%s ecc bytes %d support %d select %d define %d\n",
	    plat->name,ecc_bytes,options_support,options_selected,options_define);
    ...
    DEBUG (MTD_DEBUG_LEVEL0,"chip: page %d block %d size %lld\n"
        "oob: size %d\n"
        "mtd: page %d block %d size %lld ce %d internal chip %d plane %d \n",
        (1<<chip->page_shift),(1<<chip->phys_erase_shift),chip->chipsize,
        aml_chip->oob_size,
        mtd->writesize,mtd->erasesize,mtd->size,aml_chip->chip_num,aml_chip->internal_chipnr,aml_chip->plane_num);
...
int aml_nand_init(struct aml_nand_chip *aml_chip)
{
		...
		if (!strncmp((char*)plat->name, NAND_BOOT_NAME, strlen((const char*)NAND_BOOT_NAME))) {
			...
			switch (mtd->oobsize) {
				...
				default:
					...
					chip->ecc.layout = &aml_nand_oob_default;
					chip->ecc.layout->eccbytes = mtd->oobsize - (chip->ecc.layout->oobfree[0].length * (aml_chip->chip_num + aml_chip->plane_num));
					printk("use default eccbytes %d\n", chip->ecc.layout->eccbytes);
					break;
			}
		}
		*/
		chip->ecc.layout = &aml_nand_oob_default;
		chip->ecc.layout->eccbytes = chip->ecc.bytes * chip->ecc.steps * aml_chip->plane_num;
		chip->ecc.layout->oobfree[0].length = aml_chip->user_byte_mode * chip->ecc.steps * aml_chip->plane_num;        
		if(aml_chip->ops_mode & AML_INTERLEAVING_MODE){
		    chip->ecc.layout->eccbytes *= aml_chip->internal_chipnr;
		    chip->ecc.layout->oobfree[0].length *= aml_chip->internal_chipnr;
        }
	}
    ...
    DEBUG (MTD_DEBUG_LEVEL0,"oobsize %d eccbytes %d oobfree(len %d off %d) oobavail %d\n",
        mtd->oobsize,
        chip->ecc.layout->eccbytes,
    ...
    aml_chip->cache_buf = kzalloc((mtd->writesize + mtd->oobsize), GFP_KERNEL);
	if (aml_chip->cache_buf == NULL) {
		printk("no memory for flash cache buf\n");
		err = -ENOMEM;
		goto exit_error;
	}
...
exit_error:
	...
	if (aml_chip->cache_buf){
		kfree(aml_chip->cache_buf);
		aml_chip->cache_buf = NULL;
	}
