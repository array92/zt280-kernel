...
void osd_cursor_hw(s16 x, s16 y, s16 xstart, s16 ystart, u32 osd_w, u32 osd_h, int index)
{
	...
	if(CONFIG_FB_OSD1_ZTKEY_LEN > 0){
		...
		osd_hw.dispdata[OSD2].x_end = CONFIG_FB_OSD1_DEFAULT_WIDTH_VIRTUAL + CONFIG_FB_OSD1_ZTKEY_LEN -1;
		osd_hw.dispdata[OSD2].y_end = CONFIG_FB_OSD1_DEFAULT_HEIGHT - 1;
		osd_hw.pandata[OSD2].x_start = 0;
		osd_hw.pandata[OSD2].y_start = 0;	
		osd_hw.pandata[OSD2].x_end = CONFIG_FB_OSD1_DEFAULT_WIDTH_VIRTUAL + CONFIG_FB_OSD1_ZTKEY_LEN -1;
		osd_hw.pandata[OSD2].y_end = CONFIG_FB_OSD1_DEFAULT_HEIGHT - 1;
		add_to_update_list(OSD2,DISP_GEOMETRY);	
		osd_wait_vsync_hw();
		return;
	}
