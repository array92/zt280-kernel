...
static int bl_set_brightness(struct aml_bl *amlbl,int brightness)
{
#ifdef AML_BL_DBG
    printk(KERN_INFO "bl_set_brightness %d\n",brightness);
#endif
    if( brightness < 0 )
        brightness = 0;
...
#endif
    amlbl->bldev->props.brightness = brightness;
	return 0;
}

static int bl_get_brightness(struct aml_bl *amlbl)
{
	int brightness = amlbl->bldev->props.brightness;
...
#ifdef AML_BL_DBG
        printk(KERN_INFO "bl_get_brightness %d\n",brightness);
#endif
...
static int aml_bl_update_status(struct backlight_device *bd)
{
    ...
    return bl_set_brightness(amlbl,brightness);
}
...
static int aml_bl_get_brightness(struct backlight_device *bd)
{
	...
	return bl_get_brightness(amlbl);
}
