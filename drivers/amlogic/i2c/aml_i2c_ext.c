...
static struct aml_i2c aml_i2c_ddata = {
	.i2c_debug = 0,
	.master_no = MASTER_A,
};
...
{
	i2c->master_i2c_speed = plat->ext_master_i2c_speed;
	...
	if(i2c->master_no == MASTER_A){
		...
		memcpy(&i2c->sw_pins,&plat->master_a_sw_pins,sizeof(i2c->sw_pins));
	}
	else {
		...
		memcpy(&i2c->sw_pins,&plat->master_b_sw_pins,sizeof(i2c->sw_pins));
	}
}

static void aml_i2c_pinmux_master(struct aml_i2c *i2c,int en)
{
	en = !!en;
    WRITE_CBUS_REG_BITS(i2c->master_pinmux.scl_reg, en, i2c->master_pinmux.scl_bit, 1);
    WRITE_CBUS_REG_BITS(i2c->master_pinmux.sda_reg, en, i2c->master_pinmux.sda_bit, 1);
}

static int aml_i2c_pinmux_gpio_recovery(struct aml_i2c *i2c)
{
    int scl,sda;
    int count;
    ...
    printk("i2c ext entry recovery mode\n");

    //config to gpio mode

    set_gpio_val(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit, 1);
    set_gpio_val(i2c->sw_pins.sda_bank, i2c->sw_pins.sda_bit, 1);
    set_gpio_mode(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit, GPIO_OUTPUT_MODE);
    set_gpio_mode(i2c->sw_pins.sda_bank, i2c->sw_pins.sda_bit, GPIO_OUTPUT_MODE);

    for(count=0;count<16;count++){

        scl = get_gpio_val(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit);
        sda = get_gpio_val(i2c->sw_pins.sda_bank, i2c->sw_pins.sda_bit);

        printk("#%d scl %d sda %d\n",count,scl,sda);
        if(sda && scl)
            break;

        if(count == 0)
            aml_i2c_pinmux_master(i2c,0);

        if(!scl){
            set_gpio_val(i2c->sw_pins.sda_bank, i2c->sw_pins.sda_bit, 0);
            udelay(10);
            set_gpio_val(i2c->sw_pins.sda_bank, i2c->sw_pins.sda_bit, 1);
            udelay(10);
        }
        ...
        if(!sda){
            set_gpio_val(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit, 0);
            udelay(10);
            set_gpio_val(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit, 1);
            udelay(10);
        }
    ...
    if(sda && scl){
        aml_i2c_pinmux_master(i2c,1);
    }else{
        printk("i2c ext recovery failed\n");
        set_gpio_mode(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit, GPIO_INPUT_MODE);
        set_gpio_mode(i2c->sw_pins.sda_bank, i2c->sw_pins.sda_bit, GPIO_INPUT_MODE);
...
static void aml_i2c_bus_sleep(struct aml_i2c *i2c,int sleep)
{
    //config to gpio low

    printk("i2c ext bus sleep %d\n",sleep);
    if(sleep){
        set_gpio_val(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit, 0);
        set_gpio_val(i2c->sw_pins.sda_bank, i2c->sw_pins.sda_bit, 0);
        set_gpio_mode(i2c->sw_pins.scl_bank, i2c->sw_pins.scl_bit, GPIO_OUTPUT_MODE);
    ...
    aml_i2c_pinmux_master(i2c,!sleep);
}
...
static int aml_i2c_do_address(struct aml_i2c *i2c, unsigned int addr, unsigned char *buf, int rd, unsigned int len)
{	
							...
							int num)
{
	...
	unsigned int i,retry = 1;
	unsigned int ret=0;

    if(!num){
        printk("aml_i2c_ext_xfer: failed i2c message number\n");
        return ret;
    }

    if(i2c->bus_dead){
        printk("aml_i2c_ext_xfer: i2c bus dead try recovery\n");
	    i2c->bus_dead = aml_i2c_pinmux_gpio_recovery(i2c);
	    if(i2c->bus_dead)
	        return ret;
    }
	...
	if (ret == 0)
		...
		if(CONFIG_CONFIG_SDCARD_SLEEP != 10) {
			printk("[aml_i2c_ext_xfer] error ret = %d \t", ret);
			printk( "i2c ext master %s addr 0x%x buf[0] 0x%x\n", 
							i2c->master_no == MASTER_A?"A":"B", msgs->addr,msgs->buf?msgs->buf[0]:-1);
		}
		return ret;
	}
}
							...
							int num)
{
    ...
    if(!num){
        printk("aml_i2c_ext_xfer_slow: failed i2c message number\n");
        return ret;
    }

    if(i2c->bus_dead){
        printk("aml_i2c_ext_xfer_slow: i2c bus dead try recovery\n");
	    i2c->bus_dead = aml_i2c_pinmux_gpio_recovery(i2c);
	    if(i2c->bus_dead)
	        return ret;
    }
	...
	i2c->ops->stop(i2c);
        ...
        if(ret){
            ...
            AML_I2C_DBG("aml_i2c_ext_xfer_slow");
            if (p->flags & I2C_M_RD){
                AML_I2C_DBG("read ");
            }
            else {
    ...
    if (ret == 0)
        ...
        printk( "i2c master %s current slave addr is 0x%x req addr is 0x%x\n", 
                        i2c->master_no?"a":"b", i2c->cur_slave_addr,msgs->addr);
        return ret;
    }
    ...
    if (ret == 0)
    	...
    	if(CONFIG_CONFIG_SDCARD_SLEEP != 10) {
	        printk("[aml_i2c_xfer_slow] error ret = %d \t", ret);
	        printk( "i2c ext master %s current slave addr is 0x%x req addr is 0x%x\n", 
	                        i2c->master_no == MASTER_A?"A":"B", i2c->cur_slave_addr,msgs->addr);
        }
        return ret;
    }
}
...
{
	struct aml_i2c *i2c = &aml_i2c_ddata;
	return sprintf(buf, "i2c ext debug is 0x%x\n", i2c->i2c_debug);
}
			...
			struct class_attribute *attr,	char *buf)
{
	...
	printk( "i2c ext master %s current slave addr is 0x%x\n", 
						i2c->master_no?"a":"b", i2c->cur_slave_addr);
...
static int aml_i2c_probe(struct platform_device *pdev) 
{
	...
	i2c->master_no = plat->ext_master_no;
	i2c->use_pio = plat->ext_use_pio;
...
#if (defined(CONFIG_PM) && !defined(CONFIG_AML_I2C_EXT_PM_DISABLE))
static int aml_i2c_suspend(struct platform_device *dev, pm_message_t state)
{
    struct aml_i2c *i2c = &aml_i2c_ddata;

    aml_i2c_bus_sleep(i2c,1);
    return 0;
}

static int aml_i2c_resume(struct platform_device *dev)
{
    //let xfer wake bus
    /*
    struct aml_i2c *i2c = &aml_i2c_ddata;

    aml_i2c_bus_sleep(i2c,0);
    */

    return 0;
}
#else
#define aml_i2c_suspend NULL
#define aml_i2c_resume  NULL
#endif
