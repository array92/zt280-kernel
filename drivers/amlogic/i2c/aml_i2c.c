...
static int aml_i2c_pinmux_gpio_recovery(struct aml_i2c *i2c)
{
    ...
    //printk("i2c entry recovery mode\n");
							...
							int num)
{
    ...
    if(!num){
        printk("aml_i2c_xfer: failed i2c message number\n");
        return ret;
    }

    if(i2c->bus_dead){
        printk("aml_i2c_xfer: i2c bus dead try recovery\n");
							...
							int num)
{
    ...
    if(!num){
        printk("aml_i2c_xfer_slow: failed i2c message number\n");
        return ret;
    }

    if(i2c->bus_dead){
        printk("aml_i2c_xfer_slow: i2c bus dead try recovery\n");
	    i2c->bus_dead = aml_i2c_pinmux_gpio_recovery(i2c);
	    if(i2c->bus_dead)
	        return ret;
    }
...
#if (defined(CONFIG_PM) && !defined(CONFIG_AML_I2C_PM_DISABLE))
