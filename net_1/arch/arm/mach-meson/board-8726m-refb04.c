...
#else
	{102,			"exit", CHAN_4, 000, 60},						//= vol *1023/3.3v
	{125,			"menu", CHAN_4, 180, 60},
	{15,			"back", CHAN_4, 400, 60},
#endif
};
...
#elif (defined(CONFIG_SND_AML_M1_MID_WM8988))
static struct platform_device aml_audio={
        .name               = "aml_m1_audio_wm8988",
        .id                     = -1,
        .resource       =   aml_m1_audio_resource,
        .num_resources  =   ARRAY_SIZE(aml_m1_audio_resource),
};
...
EXPORT_SYMBOL_GPL(wm8988_is_hp_pluged);

void wm8988_codec_power(int enable){
	
	if( enable ){		
		WRITE_CBUS_REG_BITS(PREG_GGPIO_O, 1, 16, 1);
		WRITE_CBUS_REG_BITS(PREG_GGPIO_EN_N, 0, 16, 1);	
	} else {
		WRITE_CBUS_REG_BITS(PREG_GGPIO_O, 0, 16, 1);				
		WRITE_CBUS_REG_BITS(PREG_GGPIO_EN_N, 0, 16, 1);	
	}
	return;	
}
EXPORT_SYMBOL_GPL(wm8988_codec_power);


void wm8988_amp_power(int enable){
	
	if( enable ){		
		WRITE_CBUS_REG_BITS(PREG_GGPIO_O, 1, 15, 1);
		WRITE_CBUS_REG_BITS(PREG_GGPIO_EN_N, 0, 15, 1);	
	} else {		
		WRITE_CBUS_REG_BITS(PREG_GGPIO_O, 0, 15, 1);					
		WRITE_CBUS_REG_BITS(PREG_GGPIO_EN_N, 0, 15, 1);
	}
	return;	
}
EXPORT_SYMBOL_GPL(wm8988_amp_power);

static struct wm8988_platform_data wm8988_pdata = {
    .is_hp_pluged = &wm8988_is_hp_pluged,
};
#endif
...
//GPIOD_16
#define GPIO_UOR6X5X_PENIRQ ((GPIOD_bank_bit2_24(16)<<16) |GPIOD_bit_bit2_24(16)) 
#define GPIO_UOR6X5X_RST     (GPIOD_IDX + 16)
...
#define UOR6X5X_ADC_SHIFT 12
#ifdef CONFIG_AML_TCON_K8_1024x768
#define UOR6X5X_SWAP_XY 0
#define UOR6X5X_XPOL    0
#define UOR6X5X_YPOL    0
#elif CONFIG_AML_TCON_K8_1024x600
#define UOR6X5X_SWAP_XY 0
...
#define UOR6X5X_XMIN 0
#define UOR6X5X_XMAX (1<<UOR6X5X_ADC_SHIFT)
#define UOR6X5X_YMIN 0
#define UOR6X5X_YMAX (1<<UOR6X5X_ADC_SHIFT)
#define UOR6X5X_PMIN 0
#define UOR6X5X_PMAX (1<<UOR6X5X_ADC_SHIFT)
...
int uor6x5x_convert(int x, int y)
{
	...
	/*
    x = (x- UOR6X5X_XMIN) * UOR6X5X_XMAX / (UOR6X5X_XMAX - UOR6X5X_XMIN);
    y = (y- UOR6X5X_YMIN) * UOR6X5X_YMAX / (UOR6X5X_YMAX - UOR6X5X_YMIN);
    */
...
static struct uor6x5x_platform_data uor6x5x_pdata = {
    ...
    .abs_xmin = UOR6X5X_XMIN,
    .abs_xmax = UOR6X5X_XMAX,
    .abs_ymin = UOR6X5X_YMIN,
    .abs_ymax = UOR6X5X_YMAX,
    .abs_pmin = UOR6X5X_PMIN,
    .abs_pmax = UOR6X5X_PMAX,
...
static struct aml_i2c_platform aml_i2c_plat = {
    ...
    .ext_use_pio        = 0,
    .ext_master_i2c_speed   = AML_I2C_SPPED_400K,
    ...
    .master_a_sw_pins = {
        .scl_reg_out    = PREG_FGPIO_O,
        .scl_reg_in     = PREG_FGPIO_I,
        .scl_bit        = 21,
        .scl_oe         = PREG_FGPIO_EN_N,
        .sda_reg_out    = PREG_FGPIO_O,
        .sda_reg_in     = PREG_FGPIO_I,
        .sda_bit        = 22,
        .sda_oe         = PREG_EGPIO_EN_N,
    },
    .master_b_pinmux = {
    ...
    .master_b_sw_pins = {
        .scl_reg_out    = PREG_EGPIO_O,
        .scl_reg_in     = PREG_EGPIO_I,
        .scl_bit        = 19,
        .scl_oe         = PREG_EGPIO_EN_N,
        .sda_reg_out    = PREG_EGPIO_O,
        .sda_reg_in     = PREG_EGPIO_I,
        .sda_bit        = 20,
        .sda_oe         = PREG_EGPIO_EN_N,
    },
};
...
void set_bat_off(void)
{
	...
	return 0;
    //speaker power off
    set_gpio_val(GPIOD_bank_bit2_24(17), GPIOD_bit_bit2_24(17), 0);
    set_gpio_mode(GPIOD_bank_bit2_24(17), GPIOD_bit_bit2_24(17), GPIO_OUTPUT_MODE);  
...
static void aml_8726m_power_on_bl(void)
{
	...
	WRITE_CBUS_REG_BITS(PREG_GGPIO_EN_N, 0, 17, 1);			// snowwan 
	WRITE_CBUS_REG_BITS(PREG_GGPIO_O, 1, 17, 1);			// snowwan set GPIO19 high

	printk("power on backlight \n");						// snowwan


    aml_8726m_set_bl_level(100);
}
...
static void aml_8726m_power_off_bl(void)
{
    ...
    WRITE_CBUS_REG_BITS(PREG_GGPIO_EN_N, 0, 17, 1);			// snowwan 
    WRITE_CBUS_REG_BITS(PREG_GGPIO_O, 0, 17, 1);			// snowwan set GPIO19 low
...
static struct platform_device __initdata *platform_devs[] = {
        ...
        &audiodsp_device,
    #endif
    #if defined(CONFIG_SND_AML_M1_MID_WM8988)
        &aml_audio,
    #endif
...
static struct i2c_board_info __initdata aml_i2c_bus_info[] = {
...
#if !defined(CONFIG_TOUCHSCREEN_INDEPENDENT_I2C) && defined(CONFIG_UOR6X5X_RESISTIVE_TOUCHSCREEN)
    {
...
#ifdef CONFIG_SND_AML_M1_MID_WM8988
	{
		...
		.platform_data = (void *)&wm8988_pdata,
	},
#endif
...
void Enable_Amp_Power(void)
{

	printk("enable amp power\n");
    WRITE_CBUS_REG_BITS(PERIPHS_PIN_MUX_6, 0, 27, 1);		// for pinmux

	WRITE_CBUS_REG_BITS(PREG_EGPIO_EN_N, 0, 26, 1);			// W12 set high
	WRITE_CBUS_REG_BITS(PREG_EGPIO_O, 1, 26, 1);			// W12 set high
