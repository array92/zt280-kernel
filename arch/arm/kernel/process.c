...
void arm_machine_restart(char mode, const char *cmd)
{
	...
	*sram_vaddr = AMLOGIC_NORMAL_BOOT;
    ...
    printk("arm_machine_restart cmd = %s\r\n",cmd);
	...
	if(cmd){   
		...
		if(strcmp(cmd, "internal_update_reboot") == 0){
			*sram_vaddr = AMLOGIC_INTERNAL_UPDATE_REBOOT;
		}	
		if(strcmp(cmd, "slience_update_reboot") == 0){
			*sram_vaddr = AMLOGIC_SLIENCE_UPDATE_REBOOT;
		}	
		if(strcmp(cmd, "update_end_reboot") == 0){
			*sram_vaddr = AMLOGIC_UPDATE_END_REBOOT;
		}			
	}
	
    printk("arm_machine_restart *sram_vaddr = 0x%x\r\n",*sram_vaddr);
