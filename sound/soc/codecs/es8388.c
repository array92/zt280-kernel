...
static const struct snd_kcontrol_new es8388_snd_controls[] = {
    ...
    //SOC_DOUBLE_R("Playback Volume", ES8388_LDAC_VOL, ES8388_RDAC_VOL, 0, 192, 1),
    SOC_DOUBLE_R("Capture Volume", ES8388_LADC_VOL, ES8388_RADC_VOL, 0, 192, 1),     
};
...
static int es8388_i2c_ts_suspend(struct device *dev)
{
    struct i2c_client   *client = i2c_verify_client(dev);
    struct es8388_priv *es8388 =   dev_get_drvdata(&client->dev);
	struct snd_soc_codec *codec = &es8388->codec;

    // Power off
    printk("es8388_i2c_ts_suspend\n");
...
static int es8388_i2c_ts_resume(struct device *dev)
{
    struct i2c_client   *client = i2c_verify_client(dev);
    struct es8388_priv *es8388 =   dev_get_drvdata(&client->dev);
	struct snd_soc_codec *codec = &es8388->codec;
...
static const struct dev_pm_ops es8388_dev_pm_ops = {
	.suspend	= es8388_i2c_ts_suspend,
	.resume		= es8388_i2c_ts_resume,
};

#else
#define	es8388_i2c_ts_suspend	NULL
#define	es8388_i2c_ts_resume	NULL
#endif
			   ...
			   enum snd_soc_control_type control)
{
	...
	snd_soc_write(codec, ES8388_CHIPLOPOW1, 0xe8);
	...
	snd_soc_write(codec, ES8388_CHIPPOWER, 0x00);   // ALL Block POWER ON
    	...
    	snd_soc_write(codec, ES8388_LOUT2_VOL, 0x1f);   //  LOUT2
    	snd_soc_write(codec, ES8388_ROUT2_VOL, 0x1f);   // ROUT2
