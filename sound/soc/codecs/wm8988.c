...
extern void wm8988_amp_power(int enable);
extern void wm8988_codec_power(int enable);
 ...
 */
	...
	//wm8988->sysclk=11289600;
	//wm8988->sysclk_constraints = &constraints_112896;			// add by snowwan

	wm8988->sysclk=12288000;
	wm8988->sysclk_constraints = &constraints_12288;			// add by snowwan
	...
	snd_pcm_hw_constraint_list(substream->runtime, 0,
		SNDRV_PCM_HW_PARAM_RATE,wm8988->sysclk_constraints);
...
{
	u16 reg = snd_soc_read(codec, WM8988_PWR1) & 0xfe3e;
	switch (level) {
	case SND_SOC_BIAS_ON:
		/* vref/mid, osc on, dac unmute */
		
		snd_soc_write(codec, WM8988_PWR1, reg | 0x00c0);
	...
	case SND_SOC_BIAS_STANDBY:
		...
		snd_soc_write(codec, WM8988_PWR1, reg | 0x0141);
		break;
	case SND_SOC_BIAS_OFF:
		/* everything off, dac mute, inactive */
		//wm8987_write(codec, WM8987_ACTIVE1, 0x0);
		snd_soc_write(codec, WM8988_PWR1, 0x0001);
		break;
	}
...
/*	register pcms		*/
	...
	printk("--------------------- wm 8988 init ok -----------------------------\n");

	return ret;

pcm_err:

	printk("--------------------- wm 8988 init error -----------------------------\n");
			   ...
			   enum snd_soc_control_type control)
{
	...
	if (ret) {
		dev_err(codec->dev, "WM8988 chip not exist\n");
		goto err;
	}

	// init first mutes
	reg = snd_soc_read(codec,  WM8988_ADCDAC)  & 0xfff7;
	snd_soc_write(codec, WM8988_ADCDAC, 0x00);

	reg = snd_soc_read(codec,  WM8988_LOUTM1);
	snd_soc_write(codec, WM8988_LOUTM1, reg |  0x0150);
	
	reg = snd_soc_read(codec,  WM8988_ROUTM2);
	snd_soc_write(codec, WM8988_ROUTM2, reg |  0x0100);
	snd_soc_write(codec, WM8988_ROUTM2, reg |  0x0150);
	
	reg = snd_soc_read(codec,  WM8988_ADCTL1);
	snd_soc_write(codec, WM8988_ADCTL1, reg |  0x00c1);	// XXX:

	reg = snd_soc_read(codec,  WM8988_ADCTL2);
	
	reg = snd_soc_read(codec,  WM8988_LADCIN);
	snd_soc_write(codec, WM8988_LADCIN, reg |  /*0x0060*/0x50);	// XXX: LINP2
	
	reg = snd_soc_read(codec,  WM8988_PWR2);
	snd_soc_write(codec, WM8988_PWR1, reg | 0x0198);	// XXX: MIC BIAS
	reg = snd_soc_read(codec,  WM8988_PWR1);
	snd_soc_write(codec, WM8988_PWR1, reg | 0x00fe);	// XXX: MIC BIAS

	
	reg = snd_soc_read(codec,  WM8988_LADC);
	snd_soc_write(codec, WM8988_LADC, reg |  0x01fd);
	
	reg = snd_soc_read(codec,  WM8988_RADC);
	snd_soc_write(codec, WM8988_RADC, reg |  0x01fd);


	/* set the update bits */
	reg = snd_soc_read(codec,  WM8988_LDAC);
	snd_soc_write(codec, WM8988_LDAC, 0x01c7);
	
	reg = snd_soc_read(codec,  WM8988_RDAC);
	snd_soc_write(codec, WM8988_RDAC, 0x01c7);

	reg = snd_soc_read(codec,  WM8988_LINVOL);
	snd_soc_write(codec, WM8988_LINVOL, (reg & ~0x080) | 0x0140);
	reg = snd_soc_read(codec,  WM8988_RINVOL);
	snd_soc_write(codec, WM8988_RINVOL, (reg & ~0x080) | 0x0140);
	reg = snd_soc_read(codec,  WM8988_3D);
	snd_soc_write(codec, WM8988_3D, (reg | 0x80));
	snd_soc_write(codec, WM8988_ROUT2V, 0x17a);

	snd_soc_write(codec, WM8988_LOUT2V, 0x17a); //lzcx
	snd_soc_write(codec, WM8988_NGATE, 0xf9); //lzcx
	
	reg = snd_soc_read(codec,  WM8988_BASS);
	snd_soc_write(codec, WM8988_BASS, (reg | 0x40 ));	
