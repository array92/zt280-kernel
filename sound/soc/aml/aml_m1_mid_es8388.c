...
{    
    ...
    for (level = 0;level <= 52;level ++)
            printk("reg %02x %08x\n",level,snd_soc_read(codec,level));
    */
...
static int __init aml_m1_es8388_init(void)
{
	printk("aml_m1_es8388_init\n");
	if(audio_dai_type != 2)
		return 0;
    return platform_driver_register(&aml_m1_audio_driver);
}

static void __exit aml_m1_es8388_exit(void)
{
    platform_driver_unregister(&aml_m1_audio_driver);
}

module_init(aml_m1_es8388_init);
module_exit(aml_m1_es8388_exit);
